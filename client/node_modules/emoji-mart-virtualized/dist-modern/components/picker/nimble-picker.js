...
export default class NimblePicker extends React.PureComponent {
  ...
  handleScrollPaint(titleIndexes) {
    return ({
      ...
      if (this.SEARCH_CATEGORY.emojis) {
        ...
        Object.keys(titleIndexes).sort((a, b) => titleIndexes[a].row - titleIndexes[b].row).some((key, index) => {
  ...
  render() {
    ...
    this.getCategories().map((category, i) => {
      ...
      const recent = category.id === this.RECENT_CATEGORY.id ? recent : undefined;
      const custom = category.id === this.RECENT_CATEGORY.id ? this.CUSTOM : undefined;
      ...
      const makeGap = count => Array.from({
        length: count
      });
